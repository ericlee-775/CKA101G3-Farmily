<script setup>
// 小農後台：產地日記（部落格）列表。新增/查看/編輯 都導到獨立頁面
...
function goCreate() {
  router.push('/farmer/blog/new')
}
function goView(b) {
  router.push(`/farmer/blog/${b.blogId}`)
}
function goEdit(b) {
  router.push(`/farmer/blog/${b.blogId}/edit`)
}
...
<template>
  <main class="farmer-page">
    <header class="page-head">
      <h1>🌱 產地日記（部落格）</h1>
      <button class="btn-primary" @click="goCreate">＋ 新增</button>
    </header>

    <section class="card">
      <p v-if="loadState === 'loading'" class="hint">載入中…</p>
      <p v-else-if="loadState === 'error'" class="hint err">載入失敗：{{ errorMsg }}</p>
      <p v-else-if="loadState === 'empty'" class="hint">還沒有產地日記，點右上「＋ 新增」開始寫吧。</p>
